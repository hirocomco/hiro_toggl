...
def print_member_time_summary(member: MemberTimeTotal) -> None:
    ...
    print("=" * 50)
    print(f"Total Time:     {member.total_hours:.2f} hours ({member.total_duration_seconds} seconds)")
    print(f"Billable Time:  {member.billable_hours:.2f} hours ({member.billable_duration_seconds} seconds)")
...
def print_workspace_summary(members: List[MemberTimeTotal]) -> None:
    ...
    print("=" * 80)
    print(f"{'Name':<20} {'Total Hours':<12} {'Billable Hours':<15} {'Entries':<8} {'Billable %':<10}")
    print("-" * 80)
    ...
    for member in sorted(members, key=lambda m: m.total_hours, reverse=True):
        billable_pct = (member.billable_hours / member.total_hours * 100) if member.total_hours > 0 else 0
        
        print(f"{member.user_name:<20} {member.total_hours:<12.2f} {member.billable_hours:<15.2f} "
    ...
    print("-" * 80)
    overall_billable_pct = (total_billable / total_hours * 100) if total_hours > 0 else 0
    print(f"{'TOTAL':<20} {total_hours:<12.2f} {total_billable:<15.2f} "
          f"{total_entries:<8} {overall_billable_pct:<10.1f}%")


def example_get_specific_member_time(client: TogglClient, workspace_id: int, user_id: int):
...
def example_get_all_members_time(client: TogglClient, workspace_id: int):
    ...
    try:
        ...
        monthly_members = client.get_member_total_time(workspace_id, None, start_date, end_date)
...
def example_workspace_info(client: TogglClient):
    ...
    try:
        ...
        print(f"Current User: {user.get('fullname', 'Unknown')} ({user.get('email', 'Unknown')})")
...
def main():
    ...
    workspace_name = next((w['name'] for w in workspaces if w['id'] == workspace_id), 'Unknown')
